...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <span className="block font-sans text-[14px] uppercase tracking-wider text-ink font-semibold mb-2">
      {children}
    </span>
  );
}
      ...
      {BUS.enabled && (
        <>
          <div>
            ...
            <p className="font-sans text-[14px] text-ink mb-2 leading-snug">{BUS.idaHint}</p>
            <YesNoToggle value={busIda} onChange={onBusIda} />
          </div>

          <div>
            <FieldLabel>¿Necesitas autobús de vuelta?</FieldLabel>
            <p className="font-sans text-[14px] text-ink mb-2 leading-snug">{BUS.vueltaHint}</p>
      ...
      <div className="text-center py-6">
        ...
        {rsvpData.attending ? (
          <>
            ...
            <p className="font-sans text-[14px] text-ink mb-6 leading-relaxed">
              ¡Tenemos muchísima ilusión por vivir este día tan bonito a tu lado!
            </p>
            <div className="bg-sand/40 p-4 rounded text-left font-sans text-[14px] text-ink mb-8 space-y-2 border border-primary/5">
              <div>
                <span className="font-semibold uppercase tracking-wider text-[14px] text-ink">Invitado</span>
                <br />
                {rsvpData.guestName}
              </div>
              <div>
                <span className="font-semibold uppercase tracking-wider text-[14px] text-ink">
                  Intolerancia
                </span>
                <br />
              ...
              {BUS.enabled && (
                <div>
                  <span className="font-semibold uppercase tracking-wider text-[14px] text-ink">Autobús</span>
                  <br />
                  {resumenBus(rsvpData)}
                </div>
              )}
              ...
              {rsvpData.message && (
                <div>
                  <span className="font-semibold uppercase tracking-wider text-[14px] text-ink">Mensaje</span>
                  <br />
                  {rsvpData.message}
                </div>
              )}
          ...
          <>
            ...
            {rsvpData.message && (
              <div className="bg-sand/40 p-4 rounded text-left font-sans text-[14px] text-ink mb-8 border border-primary/5">
                <span className="font-semibold uppercase tracking-wider text-[14px] text-ink">Tu mensaje</span>
                <br />
                {rsvpData.message}
              </div>
            )}
        ...
        {onEdit && (
          <button
            type="button"
            onClick={onEdit}
            className="mt-4 w-full py-3 border border-primary/25 text-ink hover:bg-sand/50 font-sans text-[14px] uppercase tracking-[0.2em] rounded-full transition-all duration-300"
          >
      ...
      {step > 1 && (
        <div className="relative flex items-center justify-center h-6">
          <button
            type="button"
            onClick={goBack}
            className="absolute left-0 flex items-center gap-1 font-sans text-[14px] uppercase tracking-[0.15em] text-ink hover:text-ink transition-colors"
          >
            <ChevronLeft size={13} />
            Volver
          </button>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 py-4 bg-white hover:bg-sand text-ink border border-primary/20 font-sans text-[14px] uppercase tracking-[0.2em] rounded-full transition-all duration-300"
              >
                No podré asistir
              </button>
            </div>
        ...
        {step === 2 && attending === true && (
          <motion.div
            ...
            className="space-y-5"
          >
            <p className="font-sans text-[14px] uppercase tracking-wider text-ink font-semibold text-center">
              Tus datos
            </p>
            <GuestFields
        ...
        {step === 3 && attending === true && (
          ...
          >
            <div>
              <p className="font-sans text-[14px] uppercase tracking-wider text-ink font-semibold text-center mb-1">
                ¿Vendrás con acompañante?
              </p>
              <p className="font-sans text-[14px] text-ink/70 text-center mb-4">(si procede)</p>
              ...
              <p className="text-center font-sans text-[14px] text-ink mt-2">
                {companionCount === 0
                  ? 'Sin acompañantes'
                  : companionCount === 1
                    ? '1 acompañante'
            ...
            <AnimatePresence>
              {companions.map((c, i) => (
                ...
                >
                  <p className="font-sans text-[14px] uppercase tracking-wider text-ink font-semibold">
                    Acompañante {i + 1}
                  </p>
                  <GuestFields
                    nameLabel="Nombre y Apellidos"
        ...
        {step === 4 && attending === true && (
          ...
          >
            <div className="bg-sand/40 border border-primary/10 rounded p-4 font-sans space-y-3">
              <p className="text-[14px] uppercase tracking-wider font-semibold text-ink">
                Resumen de tu confirmación
              </p>
              <div className="space-y-2 text-[14px] text-ink">
                <div>
                  <span className="text-[14px] uppercase tracking-wider text-ink">Invitado</span>
                  <p className="mt-0.5">{guestName}</p>
                </div>
                <div>
                  <span className="text-[14px] uppercase tracking-wider text-ink">Intolerancia</span>
                  <p className="mt-0.5">{hasIntolerance ? intolerance || '—' : 'Ninguna'}</p>
                </div>
                {BUS.enabled && (
                  <div>
                    <span className="text-[14px] uppercase tracking-wider text-ink">Autobús</span>
                    <p className="mt-0.5">{resumenBus({ busIda, busVuelta })}</p>
                  </div>
                )}
                {companions.length > 0 && (
                  <div>
                    <span className="text-[14px] uppercase tracking-wider text-ink">
